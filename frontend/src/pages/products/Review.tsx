...
const Review: React.FC<ReviewProps> = ({ productId }) => {
  ...
  const reviewStatus = useAppSelector((store) => store.review.status);
  ...
  if (reviewStatus === Status.ERROR) {
    return <div className="text-red-600 text-center py-12">Error loading reviews</div>;
  }
        ...
        <div className="max-w-4xl mx-auto">
          <div className="space-y-4 mb-8">
            {review.length === 0 ? (
              ...
              review.map((item) => (
                <div key={item.id} className="flex items-start gap-4 p-4 bg-gray-100 rounded-md relative">
                  <div className="w-full">
                    <div className="flex justify-between items-center mb-2">
                      ...
                      {item.userId === currentUserId && (
                        <div className="relative">
                          <button onClick={() => toggleDropdown(item.id)} className="text-gray-600 hover:text-gray-800">
                    ...
                    <div className="flex space-x-1 mb-2">
                      {[...Array(5)].map((_, i) => (
                        <svg key={i} className={`w-5 h-5 ${i < Math.floor(item.rating ?? 0) ? "text-yellow-400" : "text-gray-300"}`} fill="currentColor" viewBox="0 0 20 20">
          ...
          <form onSubmit={handleSubmit} className="bg-gray-100 p-6 rounded-md space-y-4">
            <textarea
            ...
            <div className="flex items-center space-x-2">
              ...
              {[...Array(5)].map((_, i) => (
                <button key={i} type="button" onClick={() => handleRatingChange(i + 1)} className={`text-2xl ${i < rating ? "text-yellow-400" : "text-gray-300"}`}>
                  ★
                </button>
              ))}
            </div>
            <button type="submit" className="rounded-md px-4 py-2.5 text-sm bg-blue-600 hover:bg-blue-700 text-white">Submit Review</button>
          </form>
        </div>
      </div>
      ...
      {editModalOpen && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white p-6 rounded-md max-w-md w-full">
            <h3 className="text-lg font-bold text-gray-800 mb-4">Edit Review</h3>
            <form onSubmit={handleEditSubmit} className="space-y-4">
              ...
              <div className="flex items-center space-x-2">
                <span className="text-gray-800 font-semibold">Your Rating:</span>
                {[...Array(5)].map((_, i) => (
                  <button key={i} type="button" onClick={() => handleEditRatingChange(i + 1)} className={`text-2xl ${i < editRating ? "text-yellow-400" : "text-gray-300"}`}>
                    ★
                  </button>
                ))}
              </div>
              <div className="flex justify-end space-x-2">
                <button type="button" onClick={closeEditModal} className="px-4 py-2 text-sm bg-gray-200 hover:bg-gray-300 text-gray-800 rounded-md">Cancel</button>
                <button type="submit" className="px-4 py-2 text-sm bg-blue-600 hover:bg-blue-700 text-white rounded-md">Save Changes</button>
              </div>
            </form>
          </div>
        </div>
